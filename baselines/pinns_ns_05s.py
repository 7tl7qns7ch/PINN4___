...
def pde(x, u):
    ...
    eqn1 = w_vor_t + u_vel * w_vor_x + v_vel * w_vor_y - 1 / Re * (w_vor_xx + w_vor_yy) - forcing(x)
...
def eval(model, dataset, step, time_cost, offset, config):
    '''
...
def train(offset, config, args):
    ...
    points = dataset.get_boundary_points(num_x=num_boundary_points, num_y=num_boundary_points, num_t=dataset.T)
    ...
    net = dde.maps.FNN(config['model']['layers'], config['model']['activation'], 'Glorot normal')
