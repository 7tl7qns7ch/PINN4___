...
def train_ns(
    ...
    for e in pbar:
        ...
        # total loss
        loss = loss_f * f_weight + loss_ic * ic_weight + data_loss * xy_weight
        
        # only pde loss backward
        loss.backward()
        optimizer.step()
        scheduler.step()

        log_dict['train_loss'] = loss.item()
        log_dict['test_error'] = data_loss.item()
...
def subprocess(args):
    ...
    dataset = KFDataset(paths=config['data']['paths'], 
                        ...
                        # total_samples=1,                          # if just 1 example, otherwise total 100          
                        idx=args.idx,
